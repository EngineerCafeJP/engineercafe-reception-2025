CREATE VIEW public.seat_usage_daily_reports_view with(security_invoker=true) AS
  SELECT
    ...
    COUNT(CASE WHEN users.prefecture_id = 99 THEN 1 END) AS total_overseas_users, -- 海外ユーザーの延べ利用数
    COUNT(DISTINCT CASE WHEN users.prefecture_id = 99 THEN users.id END) AS unique_overseas_users, -- 海外ユーザーのユニーク数
