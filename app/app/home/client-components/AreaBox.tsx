...
export const AreaBox: React.FC<AreaBoxProps> = ({ areaName, maxCol,  seats, seatUsages, onSeatClick }) => {
  ...
  return (
    <div className="w-fit my-[1rem]">
      ...
      {seats.map((seat, index) => (
          <SeatCard key={index} seat={seat} seatUsage={seatUsages.find(usage => usage.seatId === seat.id) || null} onSeatClick={onSeatClick} />
        ))}
      </div>
    </div>
  );
